...
const OutGoingView = props => {
  ...
  useEffect(() => {
    if (pinCode.join('').length > 0) {
      jobData.BinDropOff.outGoingMass = pinCode.join('');
    }
  }, [pinCode]);
    ...
    <SafeAreaView
      ...
      <Portal>
        <Dialog visible={visible} onDismiss={() => setVisible(false)}>
          ...
          <Dialog.Content>
            <Paragraph>Take a picture of the OUTGOING truck mass</Paragraph>
          </Dialog.Content>
          <Dialog.Actions>
            <Button
              onPress={() => {
                ...
                if (pinCode.join('').length > 0) {
                  ...
                  ImagePicker.launchCamera(options, res => {
                    if (res.didCancel) {
                      ...
                      jobData.BinDropOff.outgoingPicture = res;
                      navigation.navigate('binWaste', {job: jobData});
                    }
                  });
                }
              }}>
